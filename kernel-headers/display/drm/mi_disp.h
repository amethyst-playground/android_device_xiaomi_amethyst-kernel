 ...
 *                         the screen refresh rate adaptive app refresh
 * frequency
 * @DDIC_MODE_QSYNC:  TE frequency and screen refresh rate adaptive app refresh
 * frequency
 * @DDIC_MODE_DIFF:   The surfaceflinger refresh rate is different with kernel
 * timing refresh rate
 * @DDIC_MODE_TEST:   Used for testing, or factory version with special refresh
 * rate
 */
enum ddic_mode_type {
  DDIC_MODE_NORMAL = 0,
...
/**
 * struct mi_dsi_display_sub_mode - specifies xiaomi timing parameters for dsi
 * display
 * @ddic_mode:             ddic mode.
